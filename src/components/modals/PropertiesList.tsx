...
const PropertiesList: React.FC<PropertiesListProps> = ({ visitorSessionId }) => {
    ...
    try {
      ...
      evtSource.onmessage = (event) => {
        try {
          ...
          if (data.SessionId !== visitorSessionId) return;

          const propsArray = Array.isArray(data.Properties) ? data.Properties : [];
          const matchingData = propsArray.map((p: any, i: number) => ({
            ...p,
            id: p.id || `${data.SessionId}-${i}`,
  ...
  const getConnectionStatusText = () => {
    switch (connectionStatus) {
      ...
      case 'error': return `Connection Error (Attempt ${reconnectAttempt})`;
      ...
      <div className="fixed bottom-4 right-4 bg-white border rounded-lg shadow-lg p-4 max-w-xs">
        ...
        {(connectionStatus === 'error' || connectionStatus === 'disconnected') && (
          <Button size="sm" onClick={() => {
            setReconnectAttempt(0);
            connectToSSE();
          }} className="w-full mt-2">Reconnect</Button>
        )}
      </div>
    );
  }
                ...
                >
                  {selectMode && (
                    <div className="absolute top-4 right-4 z-10">
                      <div className={`h-6 w-6 rounded-full border-2 flex items-center justify-center ${isSelected ? "bg-blue-600 border-blue-600 text-white" : "border-gray-400 bg-white"}`}>
                  ...
                  <div className="p-4 space-y-2">
                    ...
                    <p className="text-sm text-gray-600">{prop.community}{prop.subCommunity ? ` - ${prop.subCommunity}` : ""}</p>
          ...
          {selectMode && selectedProperties.length > 0 && (
            <div className="border-t pt-4 mt-4">
              <p className="text-sm text-gray-900 mb-2">
                You have selected <b>{selectedProperties.length}</b> properties involving <b>{uniqueAgents}</b> {uniqueAgents === 1 ? "agent" : "agents"}.
              </p>
              <label className="text-sm font-medium text-gray-900">Pick a date to meet/talk with the agent(s):</label>
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div className="space-y-4">
                  <div className="flex items-center text-gray-600">
                    ...
                    <span className="font-semibold text-gray-900">AED {selectedProperty.price.toLocaleString()}</span>
                  </div>
                  <div className="flex items-center text-gray-600">
                    <MapPin className="h-5 w-5 mr-2 text-blue-600" />
                    <span>{selectedProperty.community}{selectedProperty.subCommunity ? ` - ${selectedProperty.subCommunity}` : ""} ({selectedProperty.emirate})</span>
                  </div>
